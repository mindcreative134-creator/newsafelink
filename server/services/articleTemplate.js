...
export function buildHtmlArticle(title, snippet, sourceName, category, sourceUrl) {
  ...
  const posterImg = createSarkariPosterSvg(title, category, sourceName);

  return `
    <div class="sarkari-article" style="font-family: Arial, sans-serif; line-height: 1.7; color: #1e293b; max-width: 800px; margin: 0 auto;">
      <!-- Content Specific Poster / Banner -->
      <div style="text-align: center; margin-bottom: 24px;">
        <img src="${posterImg}" alt="${title}" style="width: 100%; border-radius: 14px; box-shadow: 0 10px 25px rgba(0,0,0,0.12); aspect-ratio: 16/9; object-fit: cover;" />
      </div>
